from database import SessionLocal, Vulnerabilities, EPSSsnapshot, KEVsnapshot, VEXstatements, Finding, get_rows_by_column_in, execute_select, get_latest_snapshots
from prioritization import compute_fusion_score, cvss_rank, epss_rank, cvss_epss_rank, fusion_rank, explain_fusion
from sqlalchemy import distinct, or_

def ranking_sbom(sbom_id, cvss_ranking=False, epss_ranking=False, cvss_epss_ranking=False, fusion_ranking=True):
    with SessionLocal() as session:
        
        cve_ids = execute_select(
            session = session, 
            selected_columns = [distinct(Vulnerabilities.cve_id)],
            where_conditions = [Finding.sbom_id == sbom_id],
            joins = [(Finding, Finding.vulnerability_id == Vulnerabilities.id)]
        )

        flat_cve_ids = [cve_id for (cve_id,) in cve_ids]
        
        CVSS_rows = get_rows_by_column_in(
            session=session,
            tables=[Vulnerabilities],
            filter_column=Vulnerabilities.cve_id,
            filter_values=flat_cve_ids,
            selected_columns=[Vulnerabilities.cve_id, Vulnerabilities.cvss_score]
        )

        EPSS_rows = get_latest_snapshots(
            session=session,
            table=EPSSsnapshot,
            cve_ids=flat_cve_ids,
            date_column=EPSSsnapshot.score_date,
            selected_columns=[EPSSsnapshot.cve_id, EPSSsnapshot.epss_score, EPSSsnapshot.percentile]
        )

        KEV_rows = get_latest_snapshots(
            session=session,
            table=KEVsnapshot,
            cve_ids=flat_cve_ids,
            date_column=KEVsnapshot.created_on,
            selected_columns=[KEVsnapshot.cve_id]
        )

        vex_statements = execute_select(
            session=session,
            selected_columns=[Vulnerabilities.cve_id, VEXstatements.status, VEXstatements.justification],
            where_conditions=[or_(VEXstatements.sbom_id == sbom_id, VEXstatements.sbom_id.is_(None)), Vulnerabilities.cve_id.in_(flat_cve_ids)],
            joins=[(VEXstatements, VEXstatements.vulnerability == Vulnerabilities.id)]
        )

    CVSS_map = {cve: score if score is not None else 0 for cve, score in CVSS_rows}
    EPSS_map = {cve: (score, perc) if score is not None and perc is not None else (0, 0) for cve, score, perc in EPSS_rows}
    KEV_map = {cve: True for (cve,) in KEV_rows}
    VEX_map = {cve: (status, justification) if status is not None else (None, None) for cve, status, justification in vex_statements}

    if cvss_ranking:
        cvss_rank(CVSS_map)

    if epss_ranking:
        epss_rank(EPSS_map)

    if cvss_epss_ranking:
        cvss_epss_rank(CVSS_map, EPSS_map)

    scores = []
    reasons = []
    if fusion_ranking:
        for cve in flat_cve_ids:
            KEV_status = KEV_map.get(cve, False)
            CVSS = CVSS_map.get(cve, 0)
            EPSS_prob, EPSS_perc = EPSS_map.get(cve, (0, 0))
            fusion_score = compute_fusion_score(CVSS_score=CVSS,
                                                    EPSS_probability=EPSS_prob,
                                                    EPSS_percentile=EPSS_perc,
                                                    KEV=KEV_status)

            if KEV_status:
                priority = 'Critical'
                priority_rank = 0
            elif EPSS_perc >= 0.85 or fusion_score >= 7.0:
                priority = 'High'
                priority_rank = 1
            else:
                priority = 'Medium'
                priority_rank = 2

            scores.append({
                'cve_id': cve,
                'fusion_score': fusion_score,
                'priority': priority,
                'priority_rank': priority_rank
            })
            reasons.append({
                'cve_id': cve, 
                "why-ranked": explain_fusion(KEV=KEV_status,
                                             CVSS_score=CVSS,
                                             EPSS_perc=EPSS_perc,
                                             vex_status=VEX_map.get(cve, (None, None))[0])
            })

        fusion_rank(scores,vex_statements=VEX_map)

    return scores